...
from sklearn.discriminant_analysis import LinearDiscriminantAnalysis as lda #Necessary change
...
def ReadDirectory(directory, remove_startswithdot=True, sample=None, seed=12345, deduplicate=True):
    ...
    #Updated to deal with current pandas version
    data = DataFrame.from_dict(data_dict).sort_values('id')
    if deduplicate:
...
def ReadSheet(filename, sample=None, seed=12345, deduplicate=True, delimiter=',', escapechar=None, text_colname=None, id_colname=None, date_colname=None, date_start=None, date_end=None):
    ...
    if filename.endswith('.csv'):
        data = pd.read_csv(filename, sep=delimiter, escapechar=escapechar, encoding='utf-8', dtype={id_colname: str})
    ...
    data.sort_values('id', inplace=True) #Changed to sort_values for compatability
...
class Keywords:
    ...
    def ReferenceKeywords(self, ngrams=1):
        ...
        ref_stats.sort_values('counts', ascending=False, inplace=True) #Changed for current pandas version
    ...
    def FindKeywords(self, support=10, ngrams=1):
        ...
        target_stats = target_stats.reindex(['ll', 'n1', 'n0', 'p1', 'p0','T','S','R'], axis=1) #Changed for current version of pandas
        target_stats.sort_values(ranked_by, ascending=False, inplace=True)#Changed for current version of pandas
        nontarget_stats = DataFrame(nontarget_stats, index=nontarget_wordlist)
        nontarget_stats = nontarget_stats.reindex(['ll', 'n1', 'n0', 'p1', 'p0','T','S','R'], axis=1) #Changed for current version of pandas
        nontarget_stats.sort_values(ranked_by, ascending=False, inplace=True)#Changed for current version of pandas
    ...
    def ViewDocs(self, doc_set='target', nprint=10, start=1, all_words=None, any_words=None, none_words=None, use_processed=True, csv_filename=None):
        ...
        if doc_set == 'target':
            sorted_searchnames = list(self.target_votecount.sort(ascending=False, inplace=False).index)
            docnames = [name for name in sorted_searchnames if name in self.target_docnames]
        elif doc_set == 'nontarget':
            sorted_searchnames = list(self.target_votecount.sort(ascending=True, inplace=False).index)
